...
module('Integration | Component | tile layer', function(hooks) {
  ...
  test('create and update tile layer using leafletDescriptors', async function(assert) {
  ...
  test('tile layer sends actions for events', async function(assert) {
    ...
    await render(hbs`
      <LeafletMap @zoom={{this.zoom}} @center={{this.center}} as |layers|>
        <layers.tile @url="http://{s}.tile.osm.org/{z}/{x}/{y}.png" @onLoading={{action this.loadingAction}}/>
      </LeafletMap>
    `);
  });
});
